...
export class ProductService {
    constructor() {
        ...
        this.syncInterval = 2 * 60 * 1000; // 2 minutos em milissegundos
    ...
    getProductById(id) {
        // Verifica se é necessário sincronizar com o ERP antes de retornar o produto
        this.checkAndSyncStock();
        
        const products = this.getAllProducts();
        return products.find(p => p.id === id);
    }
    
    // Método para verificar se é necessário sincronizar com o ERP
    checkAndSyncStock() {
        const now = new Date().getTime();
        
        // Se nunca sincronizou ou se já passou o intervalo desde a última sincronização
        if (!this.lastSyncTime || (now - this.lastSyncTime) > this.syncInterval) {
            this.syncStockFromERP();
            this.lastSyncTime = now;
        }
    }
    
    // Método que simula uma busca ao sistema externo de ERP
    syncStockFromERP() {
        console.log('Sincronizando estoque com o ERP...');
        
        const products = this.getAllProducts();
        let updated = false;
        
        // Simula atualização de estoque vindo do ERP
        products.forEach(product => {
            // Simula variação aleatória no estoque (entre -2 e +5)
            const stockChange = Math.floor(Math.random() * 8) - 2;
            
            // Garante que o estoque não fique negativo
            const newStock = Math.max(0, product.stock + stockChange);
            
            // Atualiza o estoque se houver mudança
        ...
        // Salva os produtos atualizados se houve alguma mudança
        if (updated) {
            this.storage.set('products', products);
            console.log('Estoque atualizado com sucesso!');
        } else {
            console.log('Nenhuma alteração de estoque necessária.');
        }
            ...
            const demoProducts = [
                {
                    id: '1',
                    name: 'Kit Jeans',
                    description: 'Kit Jeans, perfeito para ocasiões especiais.',
                    price: 159.90,
                ...
                {
                    id: '2',
                    name: 'Conjunto Black White',
                    description: 'Conjunto Black White para o dia a dia, feita com tecido de alta qualidade.',
                    price: 79.90,
                ...
                },
                {
                    ...
                    description: 'Jaqueta Jeans, estilo vintage, seminova em ótimo estado.',
                    price: 299.90,
                ...
                },
                {
                    ...
                    description: 'Vestido Elegante, ideal para looks sofisticados.',
                    price: 89.90,
                ...
                },
                {
                    ...
                    price: 99.90,
                    image: 'https://d5gag3xtge2og.cloudfront.net/producao/33927501/G/camisa-social-feminina-azul.webp',
                    category: 'Camisas',
                    sizes: ['PP', 'P', 'M', 'G'],
